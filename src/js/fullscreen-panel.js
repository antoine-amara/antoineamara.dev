export default class FullscreenPanel {
  ...
  createAndInsertMenu(menuClassName) {
    ...
    if (!sections.length)
      throw new Error(
        "[createAndInsertMenu] cannot find website sections, cannot create the menu"
      );
    if (!menuContainer)
      throw new Error(
        `[createAndInsertMenu] cannot find the menu container element, try to retrieve element with classname: ${menuClassName}`
      );
  ...
  _updateMenuOnScroll(nbPanelToScroll, scrollDirection) {
    ...
    if (
      !nbPanelToScroll ||
      nbPanelToScroll < 1 ||
      nbPanelToScroll > this.totalNumberOfPanels
    ) {
      throw new Error(
        `[_updateMenuOnScroll] the number of panel to scroll has to be between 1 and ${this.totalNumberOfPanels}, got ${nbPanelToScroll}`
      );
    }
    if (
      !scrollDirection ||
      (scrollDirection !== this.SCROLL_DIRECTIONS.down &&
        scrollDirection !== this.SCROLL_DIRECTIONS.up)
    ) {
      throw new Error(
        `[_scroll] invalid scroll direction, it can be ${this.SCROLL_DIRECTIONS.up} or ${this.SCROLL_DIRECTIONS.down}, got ${scrollDirection}`
      );
    }
  ...
  _determineScrollDirection(deltaY) {
    if (isNaN(deltaY))
      throw new Error(
        `[_determineScrollDirection] the parameter has to be a number, got ${deltaY}`
      );
  ...
  _onClickMenu(event) {
    ...
    nbScrollToDo < 0
      ? (direction = this.SCROLL_DIRECTIONS.up)
      : (direction = this.SCROLL_DIRECTIONS.down);

    this._scroll(Math.abs(nbScrollToDo), direction);
  }

  _scroll(nbPanelToScroll, scrollDirection = this.SCROLL_DIRECTIONS.up) {
    const currentPanel = document.getElementsByClassName(
      this.containerClassname
    )[0];

    if (!currentPanel)
      throw new Error(
        `[_scroll] cannot retrieve the current panel from the DOM, got currentPanel = ${currentPanel}`
      );
    if (
      scrollDirection !== this.SCROLL_DIRECTIONS.up &&
      scrollDirection !== this.SCROLL_DIRECTIONS.down
    ...
    if (typeof nbPanelToScroll !== "number")
      throw new Error(
        `[_scroll] the nbPanelToScroll has to be a number, got ${typeof nbPanelToScroll}`
      );

    let scrollSize = parseInt(
      currentPanel.style.transform.replace("translateY(", "")
    );
    const scrollValue = 100 * nbPanelToScroll;

    if (
      (scrollDirection === this.SCROLL_DIRECTIONS.down &&
        this.currentPanelIndex >= this.totalNumberOfPanels) ||
      (scrollDirection === this.SCROLL_DIRECTIONS.up &&
        this.currentPanelIndex <= 1)
    ) {
      return;
    ...
    if (!this.scrollInProgress) {
      ...
      if (this.hasMenu)
        this._updateMenuOnScroll(nbPanelToScroll, scrollDirection);
      setTimeout(() => {
        this.scrollInProgress = false;
      }, 1000);
    }
