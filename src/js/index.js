...
function _onMediaChange(desktopMediaQuery) {
  !desktopMediaQuery.matches ? _initMobileScripts() : _initDesktopScripts();
}
...
function _initMobileScripts() {
  ...
  const contactSubmitButton =
    document.getElementsByClassName("submit-contact")[0];
...
function _initDesktopScripts() {
  ...
  const blogPostContainer = document.getElementsByClassName(
    "blog-post__container"
  )[0];
  blogPostContainer.addEventListener("wheel", (e) => {
    e.stopPropagation();
  });
  ...
  const fakeShellElement = document.getElementsByClassName(
    "fake-terminal__commands"
  )[0];
  fakeShellElement.focus();

  // fake shell scrolling management
  fakeShellElement.addEventListener("wheel", (e) => {
    e.stopPropagation();
  });
...
async function loadAsyncContents() {
  ...
  iconLoader(
    "my-work",
    {
      ...
      errorMessage:
        "Cannot retrieve the content, click on the logo to see my projects on github.",
      errorOnClick: () => {
        window.open(MY_GITHUB_PROFILE_URL, "_blank");
      },
    },
    {
      fetcher,
