...
export async function submitContactForm (event) {
  ...
  if (form.checkValidity()) {
    ...
    const submitButtonLabel = document.getElementsByClassName('btn__content')[0]
    ...
    return fetcher(CONTACT_API_URL, { name, email, subject: CONTACT_SUBJECT[subject], message })
      .then(res => {
        ...
        notificationCenter.success(res.successMessage)
          .then(() => { submitButton.disabled = false })
        form.reset()
      }).catch(err => {
        submitButton.classList.toggle('button--loading')
        submitButtonLabel.innerHTML = BUTTON_LABEL
        notificationCenter.error(err.serverResponse.errorMessage)
          .then(() => { submitButton.disabled = false })
      })
  }
}
