...
export function manageFakeShell (selector) {
  ...
  fakeShellElement.addEventListener('keydown', (event) => {
    ...
    if (key === 'Backspace') {
      ...
      lastLine.innerHTML = `&gt; ${lineContent.substr(0, lineContent.length - 1)}<div class="fake-console__cursor fake-console--blink">&#x2588;</div>`
      return
    }
    ...
    if (key === 'Enter') {
      ...
      if (command === 'clear') {
        ...
        return _pushPhraseToFakeConsole(fakeShellElement, '', true, false, true)
      }

      isWriting = true
      return _executeShellCommand(fakeShellElement, shellCommands, command).then(() => { isWriting = false })
    } else {
    ...
    return elementToDisplay.reduce((previousPhrase, nextPhrase, elementIndex) =>
      previousPhrase.then(_ => {
        return elementIndex !== elementToDisplay.length - 1
          ? _pushPhraseToFakeConsole(fakeConsoleElement, nextPhrase, false, false, false, options.characterTimeout)
...
export function _readShellConf (jsonConfiguration) {
  if (!jsonConfiguration) {
    throw new Error('[_readShellConf] the shell configuration is undefined, cannot read it.')
  }
  if (!Object.keys(jsonConfiguration).length) {
    throw new Error('[_readShellConf] the shell configuration is empty, cannot read it.')
  }
...
export function _executeShellCommand (fakeConsoleElement, shellConfig, command) {
  if (!fakeConsoleElement) {
    throw new Error(`[_executeShellCommand] the console element is not defined, got: ${fakeConsoleElement}`)
  }
  if (!(fakeConsoleElement instanceof HTMLElement)) {
    throw new Error(`[_executeShellCommand] it seems the console is not a DOM element, got: ${typeof fakeConsoleElement}`)
  }
  if (!shellConfig) {
    throw new Error('[_executeShellCommand] the shell configuration is undefined, cannot read it.')
  }
  if (!Object.keys(shellConfig).length) {
    throw new Error('[_executeShellCommand] the shell configuration is empty, cannot read it.')
  }
  if (!command && command !== '') {
  ...
  if (!Object.keys(shellConfig).includes(command)) {
    return _pushPhraseToFakeConsole(fakeConsoleElement, `the command ${command} does not exist`, true, true)
  }

  return new Promise((resolve, reject) => {
    shellConfig[command].reduce((previousPhrase, nextPhrase, elementIndex) =>
      previousPhrase.then(_ => {
        return elementIndex !== shellConfig[command].length - 1
          ? _pushPhraseToFakeConsole(fakeConsoleElement, nextPhrase, false, false, false)
          : _pushPhraseToFakeConsole(fakeConsoleElement, nextPhrase, true, true, false)
      }),
    Promise.resolve()).then(_ => resolve(true))
  })
}
...
export function _parseShellCommand (fakeConsoleElement) {
  if (!fakeConsoleElement) {
    throw new Error(`[_parseShellCommand] the console element is not defined, got: ${fakeConsoleElement}`)
  }
  if (!(fakeConsoleElement instanceof HTMLElement)) {
    throw new Error(`[_parseShellCommand] it seems the console is not a DOM element, got: ${typeof fakeConsoleElement}`)
  }
  ...
  return lineToParse.innerHTML
    .replace('>', '')
    .replace('&gt; ', '')
    .replace('<div class="fake-console__cursor fake-console--blink">&#x2588;</div>', '')
    .replace('<div class="fake-console__cursor fake-console--blink">█</div>', '')
    .replace('<div class="fake-console__cursor fake-console--blink"█</div>', '')
}

export function _pushPhraseToFakeConsole (fakeConsoleElement, phrase, addCursor = false, newLine = false, tick = false, characterTimeout) {
  ...
  return new Promise((resolve, reject) => {
    ...
    _pushCharacterToFakeConsole(phraseContainer, `${tick ? '> ' : ''}<div class="fake-console__cursor">&#x2588;</div>`).then(() => { fakeConsoleElement.scrollTop = fakeConsoleElement.scrollHeight })
    )
      .then(_ => {
        ...
        if (newLine) {
          return _pushPhraseToFakeConsole(fakeConsoleElement, '', true, false, true)
        }
        if (addCursor) {
