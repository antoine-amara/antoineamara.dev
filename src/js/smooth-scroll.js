...
export function _exractTargetFromEvent(event) {
  ...
  if (!(event instanceof Event)) {
    throw new Error(
      `[_exractTargetFromEvent] the parameter is not an instance of an Event, got ${event}`
    );
  }
  ...
  if (!extractedTarget) {
    throw new Error(
      "[_exractTargetFromEvent] cannot retrieve the target from the event"
    );
  }
...
export function _getTargetPosition(targetId) {
  if (!targetId || typeof targetId !== "string") {
    throw new Error(
      `[_getTargetPosition] the target have to be a string, got ${typeof targetID} type`
    );
  }
  ...
  if (!targetHtmlElement) {
    throw new Error(
      "[_getTargetPosition] cannot find your target element in the DOM"
    );
  }
...
export function smoothScroll(event, animationType, animationDuration) {
  if (!event || !animationType || !animationDuration) {
    throw new Error(
      `[smoothScroll] wrong parameter, one of your parameter is undefined, got event=${event}, animationType=${animationType}, animationDuration=${animationDuration}`
    );
  }
  ...
  if (
    targetElementPosition + targetHtmlElement.clientHeight >=
