...
import { write404ErrorToFakeConsole, _readShellConf, _executeShellCommand, _parseShellCommand,  _pushPhraseToFakeConsole, _pushCharacterToFakeConsole } from '../fake-console';
...
describe('FakeConsole module: display fake UNIX like commands into the screen', () => {
        ...
        .then(_ => {
          ...
          fakeConsoleHtmlElements.forEach((htmlElement, index) => {
            if(index !== fakeConsoleHtmlElements.length - 1) {
              expect(htmlElement.innerHTML).toEqual(`${validTextToDisplay[index]}`)              
            } else {
              expect(htmlElement.innerHTML).toEqual(`${validTextToDisplay[index]}<div class="fake-console__cursor fake-console--blink">█</div>`)
            }
          })
        })
    })
  ...
  describe('_readShellConf', () => {
    ...
    test('it should be a function', () => expect(typeof _readShellConf).toBe('function'))
    ...
    test('it should return an object with the comand name as key and an array of array representing all lines', () => {
      ...
      expect(Object.keys(parsedFakeShellConf)).toEqual(Object.keys(fakeShellConf))
      expect(parsedFakeShellConf.main).toEqual(['Welcome to the shell'])
      expect(parsedFakeShellConf.general).toEqual(['Hello there!', 'General Kenobi!'])
    })
  })
  ...
  describe('_executeShellCommand', () => {
    ...
    test('it should be defined', () => expect(_executeShellCommand).toBeDefined())
    test('it should be a function', () => expect(typeof _executeShellCommand).toBe('function'))
    test('it should throw an error if the fakeConsoleElement is not defined', (done) => {
      expect(() => _executeShellCommand(undefined)).toThrow()
      expect(() => _executeShellCommand(getFakeConsole(), parsedFakeShellConf, fakeCommand).then(() => done())).not.toThrow()
    })
    test('it should throw an error if fakeConsoleElement is not an HTMLElement', (done) => {
      expect(() => _executeShellCommand('hello there', parsedFakeShellConf, fakeCommand)).toThrow()
      expect(() => _executeShellCommand('<div>hello there!</div>', parsedFakeShellConf, fakeCommand)).toThrow()
      expect(() => _executeShellCommand({ hello: 'there' }, parsedFakeShellConf, fakeCommand)).toThrow()
      expect(() => _executeShellCommand(getFakeConsole(), parsedFakeShellConf, fakeCommand).then(() => done())).not.toThrow()
    })
    test('it should throw an error if shellConfig parameter is not defined', (done) => {
      expect(() => _executeShellCommand(getFakeConsole(), undefined, fakeCommand)).toThrow()
      expect(() => _executeShellCommand(getFakeConsole(), null, fakeCommand)).toThrow()
      expect(() => _executeShellCommand(getFakeConsole(), parsedFakeShellConf, fakeCommand).then(() => done())).not.toThrow()
    })
    ...
    test('it should throw an error if command parameter is not defined', (done) => {
      expect(() => _executeShellCommand(getFakeConsole(), parsedFakeShellConf, undefined)).toThrow()
      expect(() => _executeShellCommand(getFakeConsole(), parsedFakeShellConf, null)).toThrow()
      expect(() => _executeShellCommand(getFakeConsole(), parsedFakeShellConf, '')).not.toThrow()
      expect(() => _executeShellCommand(getFakeConsole(), parsedFakeShellConf, fakeCommand).then(() => done())).not.toThrow()
    })
    test('it should display an error in the fake console element, if the command to execute is not in the config', () => {
      return _executeShellCommand(getFakeConsole(), parsedFakeShellConf, 'hack')
        .then(() => {
          const fakeConsoleHtml = getFakeConsole().childNodes[0].innerHTML;
    ...
    test('it should display the command content into the fake console', () => {
      return _executeShellCommand(getFakeConsole(), parsedFakeShellConf, 'general')
        .then(() => {
  ...
  describe('_parseShellCommand', () => {
    test('it should be defined', () => expect(_parseShellCommand).toBeDefined())
    test('it should be a function', () => expect(typeof _parseShellCommand).toBe('function'))
  ...
  describe('_pushPhraseToFakeConsole', () => {
    ...
    test('it should throw an error if fakeConsoleElement is not defined', (done) => {
      ...
      expect(_ => _pushPhraseToFakeConsole(getFakeConsole(), 'hello', false, false, false, 5).then(() => done())).not.toThrow()
    })
    test('it should throw an error if fakeConsoleElement is not an HtmlElement', (done) => {
    ...
    test('it should throw an error if textToDisplay is not a string', (done) => {
      ...
      expect(_ => _pushPhraseToFakeConsole(getFakeConsole(), [])).toThrow()
      expect(_ => _pushPhraseToFakeConsole(getFakeConsole(), 'Hello There !', false, 5).then(() => done())).not.toThrow()
    })
    test('it should write the phrase to the element', () => {
      return _pushPhraseToFakeConsole(getFakeConsole(), 'Hello there !', false, false, false, 5)
        .then(_ => {
          const fakeConsoleHtml = getFakeConsole().childNodes[0].innerHTML;
    ...
    test('it should write the phrase to the element with the ">" character at the begining of the sentence', () => {
      return _pushPhraseToFakeConsole(getFakeConsole(), 'Hello', false, false, true, 5)
        .then(_ => {
          const fakeConsoleHtml = getFakeConsole().childNodes[0].innerHTML;
    ...
    test('it should write the phrase with a cursor to the end if addCursor parameter is true', () => {
      return _pushPhraseToFakeConsole(getFakeConsole(), 'Hello there ', true, false, false, 5)
        .then(_ => {
          const fakeConsoleHtml = getFakeConsole().childNodes[0].innerHTML;
          ...
          const cursor = `${stringArrayElement[stringArrayElement.length - 3]} ${stringArrayElement[stringArrayElement.length - 2]} ${stringArrayElement[stringArrayElement.length - 1]}`

          expect(fakeConsoleHtml).toEqual('Hello there <div class="fake-console__cursor fake-console--blink">█</div>')
    ...
    test('it should add a new line with a ">" character at the begining and the cursor if newLine parameter is true', () => {
      return _pushPhraseToFakeConsole(getFakeConsole(), 'Hello there', true, true, false, 5)
        .then(() => {
          ...
          expect(newEmptyLine).toEqual('&gt; <div class="fake-console__cursor fake-console--blink">█</div>')
        })
    })
 })
  ...
  describe('_pushCharacterToFakeConsole', () => {
    ...
    test('it should add the character before the cursor if it exist', () => {
      ...
      fakeConsole.innerHTML = 'hello there!<div class="fake-console__cursor fake-console--blink">&#x2588;</div>'

      return _pushCharacterToFakeConsole(getFakeConsole(), 'o', 1)
        .then(_ => expect(fakeConsole.innerHTML).toEqual('hello there!o<div class="fake-console__cursor fake-console--blink">█</div>'))
    })
  })
})
