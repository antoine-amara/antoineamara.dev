...
import {
  ...
  extractTargetIdFromElementHref,
  throttle,
  iconLoader,
  fetcher,
  timeout,
...
describe("Utils module: reusable and generic function for javascript modules", () => {
  describe("getCurrentScrollPosition", () => {
    ...
    test("it should be defined", () =>
      expect(getCurrentScrollPosition).toBeDefined());
    test("it should be a function", () =>
      expect(typeof getCurrentScrollPosition).toBe("function"));
  ...
  describe("extractTargetIdFromElementHref", () => {
    test("it should be defined", () =>
      expect(extractTargetIdFromElementHref).toBeDefined());
    test("it should be a function", () =>
      expect(typeof extractTargetIdFromElementHref).toBe("function"));
  ...
  describe("iconLoader", () => {
    ...
    test("it should be a function", () =>
      expect(typeof iconLoader).toBe("function"));
    test("it should throw an error if the selector parameter is undefined or an empty string", (done) => {
      ...
      expect(() =>
        iconLoader(
          "elements-to-load",
          { render: fakeRender },
          { fetcher: fakeFetcher, apiUrl: "https://antoine.dev" }
        ).then(() => done())
      ).not.toThrow();
    });
    ...
    test("it should throw an error if the loaderOptions.render is not a function", (done) => {
      expect(() =>
        iconLoader("elements-to-load", { render: undefined })
      ).toThrow();
    ...
    test("it should throw an error if fetchOption.fetcher is not a function", (done) => {
      ...
      expect(() =>
        iconLoader("elements-to-load", { render: fakeRender }, { fetcher: 42 })
      ).toThrow();
      expect(() =>
        iconLoader(
          "elements-to-load",
    ...
    test("it should throw an error if fetchOption.apiUrl is not defined or empty string", (done) => {
      expect(() =>
        iconLoader(
          "elements-to-load",
          { render: fakeRender },
          { fetcher: fakeFetcher, apiUrl: undefined }
        )
      ).toThrow();
      expect(() =>
        iconLoader(
    ...
    test("it should display loaders element in the selected element, call the fetcher function, call render and delete hide the element if it resolve", () => {
      const loaderContainer = document.querySelector(
        ".elements-to-load .icon-loader__container"
      );

      const fetchPromise = iconLoader(
        "elements-to-load",
        { render: fakeRender },
        { fetcher: fakeFetcher, apiUrl: "https://antoine.dev" }
      );
      ...
      expect(
        document.getElementsByClassName("icon-loader__message")[0].innerHTML
      ...
      return fetchPromise.then(() => {
        ...
        expect(fakeFetcher).toHaveBeenCalledWith(
          "https://antoine.dev",
          undefined
        );
      });
    ...
    test("it should display an error message if the fetcher reject", () => {
      ...
      const fetchPromise = iconLoader(
        "elements-to-load",
        { render: fakeRender },
        { fetcher: fakeErrorFetcher, apiUrl: "https://antoine.dev" }
      );
      ...
      return fetchPromise.then(() => {
        ...
        expect(
          document.getElementsByClassName("icon-loader__message")[0].innerHTML
        ).toEqual("an error occured when loading content");
        expect(fakeErrorFetcher).toHaveBeenCalledTimes(1);
        // the second parameter is undefined because we don't pass any fetchBody to our fetcher
        expect(fakeErrorFetcher).toHaveBeenCalledWith(
          "https://antoine.dev",
          undefined
        );
      });
  ...
  describe("fetcher: function to fetch an URL and return the response as JSON (without authentication)", () => {
    ...
    const fakeJsonResponse = {
      successMessage:
        "un email a été envoye à notre équipe, nous vous répondrons dans les meilleurs délais.",
    };
    ...
    test("it should throw an error if the url is malformed", async () => {
      try {
        ...
        expect(err).toEqual(
          new Error(
            '[fetcher] the url you provide is not a valid https url, got "lacuisinedepaulo.com/contact"'
          )
        );
      }
      ...
      try {
        ...
        expect(err).toEqual(
          new Error(
            '[fetcher] the url you provide is not a valid https url, got "hello there"'
          )
        );
      }
      ...
      try {
        ...
        expect(err).toEqual(
          new Error(
            '[fetcher] the url you provide is not a valid https url, got "lacuisinedepaulo/contact"'
          )
        );
      }
      ...
      try {
        ...
        expect(err).toEqual(
          new Error(
            '[fetcher] the url you provide is not a valid https url, got "http://lacuisinedepaulo"'
          )
        );
      }
    });
    test("it should throw a FetchError if server respond with an error code (4XX/5XX)", () => {
      ...
      const error = new FetchError(
        "[fetcher] receive error code 500 from HTTP request",
        { ok: false, status: 500 },
        fakeJsonResponse
      );

      return fetcher(
        "https://north-fr-lacuisinedepaulo.cloudfunction.localhost/contact"
      ).catch((err) => {
        expect(err).toEqual(error);
      });
    });
  ...
  describe("timeout: function  who wait N milliseconds (waitingTime) and resolve", () => {
    ...
    test("it should return a Promise", () =>
      expect(timeout(42).then).toBeDefined());
