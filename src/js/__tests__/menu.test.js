...
describe('Menu module: manage an hamburger menu', () => {
  ...
  describe('Menu constructor', () => {
    ...
    test('it should be a class (function)', () => expect(typeof Menu).toBe('function'))
  ...
  describe('setEventListener', () => {
    test('it should be defined', () => expect(fakeMenu.setEventListener).toBeDefined())
    test('it should be a function', () => expect(typeof fakeMenu.setEventListener).toBe('function'))
    ...
    test('it should determine and set the callback event to apply for each menu elements', () => {
      ...
      menuItems.forEach(element => { element.addEventListener = fakeAddEventListener })

      setEventListener()

      // it apply event listener on the menu icon and on each menu items
      expect(fakeAddEventListener.mock.calls.length).toEqual(menuItems.length + 1)
      expect(fakeDetermineMenuElement.mock.calls.length).toEqual(menuItems.length)
    })
  ...
  describe('_determineMenuElementClickEvent', () => {
    ...
    `
      ...
      expect(() => _determineMenuElementClickEvent(invalidMenuElement)).toThrow()
    })
    test('it should check if the target href is an anchor and return the _onMobileMenuElementAnchorClickEvent callback', () => {
      const { _determineMenuElementClickEvent, _onMobileMenuElementAnchorClickEvent } = fakeMenu
      const anchorMenuElement = document.getElementById('anchor-element')

      const clickEventCallback = _determineMenuElementClickEvent(anchorMenuElement)
    ...
    test('it should check if the target href is a html filename concatenated to an anchor and return the _onMobileMenuElementAnchorClickEvent callback', () => {
      const { _determineMenuElementClickEvent, _onMobileMenuElementAnchorClickEvent } = fakeMenu
      const anchorAndFileElement = document.getElementById('anchorandfile-element')

      const clickEventCallback = _determineMenuElementClickEvent(anchorAndFileElement)

      expect(clickEventCallback).toEqual(_onMobileMenuElementAnchorClickEvent)
    })

    test('it should check if the target href is an html file and return the _onMobileMenuElementPageClickEvent callback', () => {
      const { _determineMenuElementClickEvent, _onMobileMenuElementPageClickEvent } = fakeMenu
  ...
  describe('_updateActiveElement', () => {
    test('it should be defined', () => expect(fakeMenu._updateActiveElement).toBeDefined())
    test('it should be a function', () => expect(typeof fakeMenu._updateActiveElement).toBe('function'))
