...
class Menu {
  constructor() {
    ...
    if (!this.menu)
      throw new Error(`[menu] the menu element is not found, got ${this.menu}`);
    if (!this.overlay)
      throw new Error(
        `[menu] the overlay element is not found, got ${this.overlay}`
      );
    ...
    this._determineMenuElementClickEvent =
      this._determineMenuElementClickEvent.bind(this);
    this._scrollChangeActiveElement =
      this._scrollChangeActiveElement.bind(this);
    this._throttledScrollChangeActiveElement = throttle(
      this._scrollChangeActiveElement,
      500,
      false,
      true
    );
    this._onMobileMenuElementPageClickEvent =
      this._onMobileMenuElementPageClickEvent.bind(this);
    this._onMobileMenuElementAnchorClickEvent =
      this._onMobileMenuElementAnchorClickEvent.bind(this);
    this._extractAndUpdateActiveElement =
      this._extractAndUpdateActiveElement.bind(this);
  ...
  setEventListener(scrollUpdate = false) {
    if (!this.menuItems.length) {
      throw new Error(
        "[setEventListener] cannot initialize your menu eventListener because it has 0 elements"
      );
    }
    ...
    if (scrollUpdate)
      document.addEventListener(
        "scroll",
        this._throttledScrollChangeActiveElement
      );
  ...
  _determineMenuElementClickEvent(element) {
    ...
    if (
      anchorRegex.test(menuTarget) ||
      filenameWithanchorRegex.test(menuTarget)
    ) {
    ...
    throw new Error(
      `[_determineMenuElementClickEvent] invalid menu element href, it has to be an anchor or an html filepath, got ${menuHref}`
    );
  }
  ...
  _scrollChangeActiveElement() {
    ...
    this.menuItems.forEach((element) => {
      ...
      const domElement = document.getElementById(
        extractTargetIdFromElementHref(element)
      );
      if (domElement) lastAnchorElement = element;
      if (
        domElement &&
        currentScrollPosition >= domElement.offsetTop &&
        currentScrollPosition < domElement.offsetTop + domElement.clientHeight
      ) {
      ...
      if (
        window.innerHeight + currentScrollPosition + 200 >=
  ...
  _updateActiveElement(newElementActive) {
    if (!newElementActive || !(newElementActive instanceof HTMLElement)) {
      throw new Error(
        `[_updateActiveElement] the new active element has to be a DOM element, got ${typeof newElementActive}`
      );
    }
    this.menuItems.map(
      (element) =>
